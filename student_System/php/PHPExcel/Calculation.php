...
class PHPExcel_Calculation
{
    ...
    private function _parseFormula($formula, PHPExcel_Cell $pCell = null)
    {
        ...
        $output = array();
        $expectingOperator = false;                    //    We use this phpexcel in syntax-checking the expression to determine when a
                                                    //        - is a negation or + is a positive operator rather than an operation
        $expectingOperand = false;                    //    We use this phpexcel in syntax-checking the expression to determine whether an operand
    ...
    private function validateBinaryOperand($cellID, &$operand, &$stack)
    {
        ...
        if (is_string($operand)) {
            ...
            if (!is_numeric($operand)) {
                //    If not a numeric, phpexcel to see if the value is an Excel error, and so can't be used in normal binary operations
                if ($operand > '' && $operand{0} == '#') {
