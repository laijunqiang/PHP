...
//  header — 发送原生 HTTP 头
    ...
    if (strlen($name)>20)
        echo "<script>alert('账号不能超过20位，请重新输入');window.location.href='../html/index.html';</script>";
//  先查看输入的账号，再根据输入的账号去数据库查密码，最后验证密码是否和mysql里面的密码相匹配
    ...
    $query1="select * from t_admin where name='$name' and password='$password'";
    $query2="select * from t_teacher where number='$name' and password='$password' and delete_time='0'";
