...
//      如果查询出错，find方法返回false，如果查询结果为空返回NULL，查询成功则返回一个关联数组（键值是字段名或者别名）。
    ...
    public function updateOil(){
        ...
        foreach ($name as $v){
            if ($v==$_POST['name'])
                return show(0,'油品名已存在，请重新修改');
        }

        $ret = D('Oil')->updateOil($_POST);
//        save方法的返回值是影响的记录数，如果返回false则表示更新出错
        if(!$ret) {
            return show(0,'修改油品失败，请重新修改');
        }else {
            $index = A('Log');
            $number=$_POST['number'];
            if (session('adminUser.account')!=null) {
                $index->addLog("修改油品编号为：$number 的油品", session('adminUser.account'));
            }else {
                $index->addLog("修改油品编号为：$number 的油品", session('User.name'));
            }
            return show(1, '修改油品成功');
        }
    }
    //删除油品处理
...
//        save方法的返回值是影响的记录数，如果返回false则表示更新出错
        if(!$ret) {
            ...
            $id=$_POST['id'];
            if (session('adminUser.account')!=null) {
                $index->addLog("删除油品ID为：$id 的油品", session('adminUser.account'));
            }else {
                $index->addLog("删除油品ID为：$id 的油品", session('User.name'));
            }
            return show(1, '删除成功');
        }
    }
    ...
    public function addOil(){
        ...
        if(!$ret) {
            ...
            $number=$_POST['number'];
            if (session('adminUser.account')!=null) {
                $index->addLog("生成油品编号为：$number 的油品", session('adminUser.account'));
            }else {
                $index->addLog("生成油品编号为：$number 的油品", session('User.name'));
            }
            return show(1, '录入成功');
        }
    }
